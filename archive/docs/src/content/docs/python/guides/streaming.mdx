...
<Aside type="caution">
Requires a Bloomberg B-PIPE environment and the applicable service entitlements. It is not available from a standard Terminal-only setup, and `BlpBPipeError` is raised when the service or entitlements are unavailable.
</Aside>
