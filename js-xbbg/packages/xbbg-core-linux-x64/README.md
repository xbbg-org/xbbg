...
Platform-specific prebuilt native `napi_xbbg.node` addon used by `@xbbg/core`. This package supports the `linux x64` target. npm installs it automatically as an optional dependency of `@xbbg/core` on matching platforms.
...
Direct installation of `@xbbg/core-linux-x64` is only recommended for diagnostics or offline packaging workflows.
