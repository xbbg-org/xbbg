...
- **`_convert_backend()` relies on pandas public API**: Replaced the pandas-DataFrame detection from a `hasattr(nw_df, "_mgr")` sniff (pandas internal attribute, churns across 2.x releases) to `isinstance(nw_df, pd.DataFrame)` via a deferred import. Same behavior, stable across pandas versions. The polars branch similarly now uses `isinstance(native, pl.DataFrame)` instead of a `hasattr(native, "to_arrow")` capability check.
