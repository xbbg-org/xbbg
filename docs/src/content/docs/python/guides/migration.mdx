...
| Field | Default | Notes |
|-------|---------|-------|
| `host` | `"localhost"` | Bloomberg server hostname. Aliases: `server`, `server_host` |
...
| `num_start_attempts` | (engine default) | Session retry count. Alias: `max_attempt` |
| `auto_restart_on_disconnection` | `True` | Auto-reconnect on disconnect. Alias: `auto_restart` |

Several legacy aliases are accepted for backward compatibility — `server`, `server_host`,
`server_port`, `max_attempt`, `auto_restart`, `max_recovery`, `retry_max`, `retry_delay`,
`retry_backoff` — and are normalized to the canonical names internally.
...
5. **Remove `connect()` / `disconnect()` calls**
   ...
   your first request. `configure()` accepts the same field names as `connect()` plus the legacy
   `server_host` / `server_port` / `max_attempt` / `auto_restart` aliases.
