...
This guide shows how to make a request from an environment that is already authorized for Bloomberg use.

<Aside type="caution">
  You need an active Bloomberg Terminal, B-PIPE, or ZFP environment with any required Bloomberg permissions, entitlements, credentials, and applicable terms to run these examples.
</Aside>
...
`start_date` and `end_date` accept ISO 8601 strings, `YYYYMMDD` strings,
...
See the [Dates and Datetimes guide](./guides/dates/) for the full accepted
set, override-path normalization, and timezone semantics.
