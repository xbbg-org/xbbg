...
configure(
    ...
    overflow_policy='drop_oldest',     # what to do when the channel is full
)
```
...
- [API Reference — `subscribe` / `asubscribe`](/xbbg/api/blp/#asubscribe)
- [API Reference — `stream` / `astream`](/xbbg/api/blp/#astream)
- [Engine Configuration](/xbbg/guides/configuration/) for `subscription_pool_size`, global flush/overflow defaults
