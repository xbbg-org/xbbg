...
impl ElementRef {
    ...
    pub fn has(&self, name: &str, exclude_null: bool) -> bool {
        ...
        let rc = unsafe {
            if exclude_null {
                blpapi_sys::blpapi_Element_hasElementEx(
                    self.ptr,
                    cname.as_ptr(),
                    std::ptr::null(),
                    1,
    ...
    pub fn get_element_at(&self, index: usize) -> Option<ElementRef> {
        let mut el_ptr: *mut blpapi_sys::blpapi_Element_t = std::ptr::null_mut();
        let rc = unsafe { blpapi_sys::blpapi_Element_getElementAt(self.ptr, &mut el_ptr, index) };
        if rc == 0 && !el_ptr.is_null() {
            Some(ElementRef { ptr: el_ptr })
        } else {
            None
    ...
    pub fn get_value_as_element(&self, index: usize) -> Option<ElementRef> {
        let mut el_ptr: *mut blpapi_sys::blpapi_Element_t = std::ptr::null_mut();
        let rc =
            unsafe { blpapi_sys::blpapi_Element_getValueAsElement(self.ptr, &mut el_ptr, index) };
        if rc == 0 && !el_ptr.is_null() {
            Some(ElementRef { ptr: el_ptr })
        } else {
            None
    ...
    pub fn get_value_as_string(&self, index: usize) -> Option<String> {
        ...
        let rc = unsafe { blpapi_sys::blpapi_Element_getValueAsString(self.ptr, &mut buf, index) };
        if rc == 0 && !buf.is_null() {
            Some(
    ...
    pub fn get_value_as_float64(&self, index: usize) -> Option<f64> {
        let mut buf: f64 = 0.0;
        let rc = unsafe { blpapi_sys::blpapi_Element_getValueAsFloat64(self.ptr, &mut buf, index) };
        if rc == 0 {
            Some(buf)
        } else {
            None
    ...
    pub fn get_value_as_int64(&self, index: usize) -> Option<i64> {
        let mut buf: i64 = 0;
        let rc = unsafe { blpapi_sys::blpapi_Element_getValueAsInt64(self.ptr, &mut buf, index) };
        if rc == 0 {
            Some(buf)
        } else {
            None
    ...
    pub fn get_value_as_datetime(
        &self,
        index: usize,
    ) -> Result<Option<chrono::DateTime<chrono::Utc>>> {
        use blpapi_sys::blpapi_Datetime_t;
        let mut dt: blpapi_Datetime_t = unsafe { std::mem::zeroed() };
        let rc = unsafe { blpapi_sys::blpapi_Element_getValueAsDatetime(self.ptr, &mut dt, index) };
        if rc != 0 {
            return Ok(None);
        }
