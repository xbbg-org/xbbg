...
pub fn execute_field_search_arrow(
    ...
    let blp_request = service.create_request("FieldSearchRequest")?;
        ...
        unsafe {
            ...
            let rc = blpapi_sys::blpapi_Element_setElementString(root_el, k_search_spec.as_ptr(), std::ptr::null(), c_search.as_ptr());
        ...
        if let Some(field_data_array) = root.get_element("fieldData") {
            ...
            for field_idx in 0..num_fields {
                if let Some(field_el) = field_data_array.get_value_as_element(field_idx) {
                    ...
                    let field_id = field_el.get_element("id")
                    ...
                    if let Some(field_info) = field_el.get_element("fieldInfo") {
                        ...
                        field_names.push(field_info.get_element("mnemonic").and_then(|el| el.get_value_as_string(0)));
                        field_types.push(field_info.get_element("ftype").and_then(|el| el.get_value_as_string(0)));
                        descriptions.push(field_info.get_element("description").and_then(|el| el.get_value_as_string(0)));
...
pub fn execute_field_info_arrow(
    session: &Session,
    req: &FieldInfoRequest,
) -> Result<RecordBatch> {
    req.validate().map_err(|e| crate::BlpError::InvalidArgument {
        detail: e.to_string(),
    })?;
    ...
    unsafe {
        ...
        let mut rc = blpapi_sys::blpapi_Element_getElement(root_el, &mut el_field_ids, k_id.as_ptr(), std::ptr::null());
        if rc != 0 || el_field_ids.is_null() {
            // Fallback to "fieldIds"
            let k_field_ids = CString::new("fieldIds").unwrap();
            rc = blpapi_sys::blpapi_Element_getElement(root_el, &mut el_field_ids, k_field_ids.as_ptr(), std::ptr::null());
        }
        if rc == 0 && !el_field_ids.is_null() {
            for field_id in &req.field_ids {
                let c_field_id = CString::new(field_id.as_str()).unwrap();
                let rc = blpapi_sys::blpapi_Element_setValueString(el_field_ids, c_field_id.as_ptr(), blpapi_sys::BLPAPI_ELEMENT_INDEX_END as usize);
                if rc != 0 {
                    return Err(crate::BlpError::InvalidArgument {
        ...
        if let Some(field_data_array) = root.get_element("fieldData") {
            ...
            for field_idx in 0..num_fields {
                if let Some(field_el) = field_data_array.get_value_as_element(field_idx) {
                    ...
                    if let Some(field_info) = field_el.get_element("fieldInfo") {
                        field_ids.push(field_id);
                        mnemonics.push(field_info.get_element("mnemonic").and_then(|el| el.get_value_as_string(0)));
                        ftypes.push(field_info.get_element("ftype").and_then(|el| el.get_value_as_string(0)));
                        descriptions.push(field_info.get_element("description").and_then(|el| el.get_value_as_string(0)));
                        categories.push(field_info.get_element("categoryName").and_then(|el| {
                            // categoryName is an array, get first value if available
                            if el.num_values() > 0 {
