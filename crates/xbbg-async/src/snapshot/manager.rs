...
impl SnapshotTemplateManager {
    ...
    pub fn create(
        &self,
        subscription: &str,
        status_cid: &CorrelationId,
    ) -> Result<SnapshotTemplateHandle, BlpAsyncError> {
        // Route for status CID
        info!(?status_cid, subscription, "snapshot: create template");
        let rx = self.router.register_route(status_cid);
        let tmpl = self
            .core
            .create_snapshot_request_template_with_cid(subscription, Some(status_cid))
            .map_err(|e| {
                BlpAsyncError::Internal(format!("create_snapshot_request_template: {e:?}"))
            })?;
    ...
    pub fn create_many(
        &self,
        subs: &[String],
        make_status_cid: impl Fn(&str) -> CorrelationId,
    ) -> Result<Vec<SnapshotTemplateHandle>, BlpAsyncError> {
